...
- External JavaScript wird meist in dedizierten Dateien ausgelagert. Solche Dateien müssen mit der Extension **.js** inkludiert werden.
...
- Beispiel:
- ```js
  element = document.getElementByID("greeter");	// fetch element
...
- Um im DOM Tree Knoten zu finden können wir auch weitere Node-Properties nutzen:
	...
	- `previousSibling` - das vorherige **Element mit gleichem Rang**
...
- ```html
  ...
  \___/ \________________/\_____/ \_______/  \___/
    |           |            |        |        |
  scheme    authority       path    query   fragment
  ```
-
- # Browser
- Browser sind massive Software Produkte die meist auf mehreren Millionen Zeilen Code basieren (Chrome Broswer ~7 mio). Sie ermöglichen es einem Nutzer mit dem World Wide Web zu interagieren und stellen die Hauptschnittstelle zum Web dar.
- ![image.png](../assets/image_1706532954562_0.png)
- ## User Interface
- Das User Interface enthält die Addressleiste (URL bar), die Vorwärts-/Rückwartsknöpfe, die Lesezeichen, etc. welche im Browser UI Verfügbar sind, aber **enthält nicht** die Website selber.
- ![image.png](../assets/image_1706534260881_0.png)
- ## Browser Engine
- Agiert als eine Art zentraler Taskmaster, welcher bestimmt welche Aktionen vom UI und der Rendering Engine ausgeführt werden müssen.
- ## Networking
- Ist ein Bestandteil des Browsers der Netzwerkkommunikation wie z.B. HTTP-Requests durchführt.
- ## JavaScript Interpreter
- Wenn ein Dokument JavaScript enthält wird der JavaScript Interpreter genutzt um den Code zu parsen und executen.
- ## Rendering Engine
- Die Rendering Engine ist dafür Verantwortlich angeforderte Dokumente anzuzeigen. Wenn ein Browser ein HTML Dokument anfordert wird es an die Rendering Engine weitergegeben um den HTML und CSS code zu verarbeiten und im Webbrowser darzustellen.
- Der "main flow" einer Rendering Engine.
- ![image.png](../assets/image_1706534482345_0.png){:width 400}
-
-
- Die Funktionsweise einer Rendering Engine wird in mehrere Schritte unterteilt:
- ![image.png](../assets/image_1706533890795_0.png)
	- Das HTML Dokument wird geparst und ein DOM Tree für die Elemente erstellt
	  logseq.order-list-type:: number
	- Basierend auf dem DOM Tree wird ein Render Tree gebaut der Elemente in "Rechtecke" mit visuellen Attributen umwandelt
	  logseq.order-list-type:: number
	- Das Layout des Render Trees wird formattiert, d.h. den einzenlen Nodes werden exakte Koordinaten im Bildschirm zugewiesen
	  logseq.order-list-type:: number
	- Die Rendering Engine geht den Render Tree durch und "malt" die einzelnen "Rechtecke" im Browser
	  logseq.order-list-type:: number
-
- ## Browser Fehlertoleranz
- Teil der Rendering Engine ist ein HTML Parser, welcher ein HTML Dokument zu einem Parse Tree gliedert.
- Um den Nutzer davor zu schützen, dass der Browser plötzliche abstürzt weil Entwickler schlchten HTML Code geschrieben haben ist ein Browser meist sehr Fehlertolerant.
- Der HTML Parser einer Rendering Engine kann verschiedene HTML Fehler ausbessern. Wenn ein end-Tag vergessen wird oder Rechtschreibfehler in die Tags einfließen kann ein HTML Parser oft dennoch "verstehen" was im Code gemeint wird. Wo ein C-Compiler Fehler melden würde kann der HTML Parser die Fehler ausbessern oder zumindest ignorieren ohne dass der Browser abstürzt.
- Beispiele:
	...
	- **Zertifikatprüfung durch den Client**
	  logseq.order-list-type:: number
		- Client prüft das Zertifikat auf **Gültigkeit und Authentizität**
		  logseq.order-list-type:: number
		- **Client** verwendet **öffentliche Schlüssel** des Zertifikat **um** sichere **Verbindung** **herzustellen**
		  logseq.order-list-type:: number
	- **Schlüsselaustausch**
	  logseq.order-list-type:: number
...
- ```js
  ...
  ```
	...
	- `SSLCertificateFile` und `SSLCertificateKeyFile` verweisen auf die Zertifikatdatei und Private Key Dateien
